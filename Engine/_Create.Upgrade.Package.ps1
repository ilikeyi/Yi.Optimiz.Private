<#
	...
	.LINK
	 https://github.com/ilikeyi/Yi.Optimiz.Private
...
<#
  ...
  .压缩包保存到
#>
$UpdateSaveTo = "$([Environment]::GetFolderPath("Desktop"))\Yi.Optimiz.Private.Upgrade.Package"

<#
	.Archive temporary directory
	.压缩包临时目录
#>
$TempFolderUpdate = "$([Environment]::GetFolderPath("MyDocuments"))\Temp.Yi.Optimiz.Private.Upgrade.Package"
...
Function Update_Create_UI
{
   ...
   buildstring: $((Get-Module -Name Engine).Version.ToString()).bs_release.2024.04.18`n"
	...
	$GUIUpdateCreateASC = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($GUIUpdateCreateASC.Checked) {
				...
				Save_Dynamic -regkey "Yi.Optimiz.Private" -name "IsPGP" -value "True" -String
			} else {
				$GUIUpdateCreateASCPanel.Enabled = $False
				Save_Dynamic -regkey "Yi.Optimiz.Private" -name "IsPGP" -value "False" -String
			}
		}
	}
			...
			if ($GUIUpdateCreateASC.Enabled) {
				if ($GUIUpdateCreateASC.Checked) {
					if ([string]::IsNullOrEmpty($GUIUpdateCreateASCSign.Text)) {
						...
						Save_Dynamic -regkey "Yi.Optimiz.Private" -name "PGP" -value $GUIUpdateCreateASCSign.Text -String
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Engine).Author)\Yi.Optimiz.Private" -Name "PGP" -ErrorAction SilentlyContinue) {
		$GUIUpdateCreateASCSign.Text = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Engine).Author)\Yi.Optimiz.Private" -Name "PGP" -ErrorAction SilentlyContinue
	}
	...
	if (Test-Path -Path $Verify_Install_Path -PathType leaf) {
		...
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Engine).Author)\Yi.Optimiz.Private" -Name "IsPGP" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Engine).Author)\Yi.Optimiz.Private" -Name "IsPGP" -ErrorAction SilentlyContinue) {
				"True" {
...
@"
{
	...
	"url": "$((Get-Module -Name Engine).HelpInfoURI)/download/solutions/update/Yi.Optimiz.Private/latest.zip"
}
